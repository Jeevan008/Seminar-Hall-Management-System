...
char fname7[30]="Kuvempu_seminar_hall.txt";
...
void seminar1::display(char file_name[30])
{
	...
	cout << '|' << setw(15) << "s.no" << '|'<< setw(15) << "date" << '|'<< setw(15) << "time" << '|'<< setw(15) << "branch" << '|'<< setw(15) << "description" << '|'<< setw(15) << "id" << '|' << " \n";
	while(1){
	f.read((char*)&r,sizeof(r));
	if(!f.eof())
{	//cout.flush();
	cout << '|' << setw(15) << c++ << '|'<< setw(15) << r.date << '|'<< setw(15) << r.time << '|'<< setw(15) << r.branch << '|'<< setw(15) << r.discription << '|'<< setw(15) ; official(r.id);cout << '|' << " \n";
...
for(int k=0;k<i;k++)
{
	if(id[k]==a)
	{
	cout << '|' << setw(4) << c++ << '|'<< setw(15) << date_list[k] << '|'<< setw(10) <<time_list[k] << '|'<< setw(10) << branch[k] << '|'<< setw(15) << description[k] << '|'<< setw(15) ; official(r.id);cout << '|' << " \n";
...
char get_time[15];
	...
	again_time_slot:cout<<"\n\t\t\t\tenter the time slot you want to do modification:\t";
	cin>>get_time;
...
for(int j=0;j<i;j++)
{
	if((id[j]>a && id[j]<10) && strcmp(date_check,date_list[j])==0 && strcmp(time_check,time_list[j])==0)
	{
		...
		cout << '|' << setw(4) << "s.no" << '|'<< setw(15) << "date" << '|'<< setw(10) << "time" << '|'<< setw(10) << "branch" << '|'<< setw(15) << "description" << '|'<< setw(16) << "id" << '|' << endl;
	cout << '|' << setw(4) << c++ << '|'<< setw(15) << date_list[j] << '|'<< setw(10) <<time_list[j] << '|'<< setw(10) << branch[j] << '|'<< setw(15) << description[j] << '|'<< setw(15) ; official(r.id);cout << '|' << " \n";
goto update_failed;
...
f.close();
fstream ff;
ff.open(file_name,ios::out);
ff.close();
fstream fff;
fff.open(file_name,ios::app);
